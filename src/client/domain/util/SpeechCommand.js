...
export default class SpeechCommand extends EventEmitter {
  ...
  onResult(recognitionResult) {
    const MAX_LETTERS_TOO_MUCH = 200;
    const command = this.commands
      .find(commandToCheck => recognitionResult.includes(commandToCheck));

    if (!command)
      return;

    const stringTooMuch = (recognitionResult.substr(0, recognitionResult.indexOf(command)) +
      recognitionResult.substr(
        recognitionResult.indexOf(command) + command.length, recognitionResult.length)
    );
